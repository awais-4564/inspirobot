...
import '../Core/languageservice.dart';
...
class TranslateProvider with ChangeNotifier {
  ...
  final Map<String, String> langCodes = {
    ...
    'Yoruba': 'yo',
    'Zulu': 'zu',
    'Pakistan': 'pk', // Pakistan added here
  };
  ...
  Future<void> googleTranslate() async {
    if (textC.text.trim().isEmpty || _to.isEmpty) {
      return;
    }
    ...
    try {
      // Fetching translation using language codes
      resultC.text = await APIs.googleTranslate(
        from: _from.isNotEmpty ? langCodes[_from]! : 'auto',
        to: langCodes[_to]!,
        text: textC.text,
      );
    } catch (e) {
      resultC.text = 'Translation failed: $e';
    }
  ...
  List<String> get lang => langCodes.keys.toList();
}
